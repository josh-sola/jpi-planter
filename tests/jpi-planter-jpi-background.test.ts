...
test("jpi-background tasks:v1 is a full replace-set, including clearing to empty", () => {
  const populated = jpiBackgroundTasksLevel(
    jpiBackgroundTasksPayload([
      { kind: "task", id: "abc", status: "running" },
  ...
  assert.equal(
    jpiBackgroundTasksLevel(jpiBackgroundTasksPayload([{ kind: "task", id: "x" } as never])),
    undefined,
  );
});
...
test("jpi-background terminal broadcasts resolve to a namespaced id only when terminal", () => {
  assert.equal(
    jpiBackgroundTerminalId(
      jpiBackgroundTerminalPayload({ kind: "task", id: "abc", status: "failed" }),
    ),
    namespaced("abc"),
  );
  assert.equal(
    jpiBackgroundTerminalId(
      jpiBackgroundTerminalPayload({ kind: "monitor", id: "mon", status: "running" }),
    ),
    undefined,
  );
  assert.equal(
    jpiBackgroundTerminalId({
      schema: "wrong",
      task: { kind: "task", id: "abc", status: "failed" },
    }),
    undefined,
  );
});
...
test("the monitor is push-only: one initial request, replace-set updates, and deduped terminal removals", () => {
  ...
  const monitor = new JpiBackgroundTaskMonitor(
    events,
    () => "req-1",
  ...
  events.emit(
    "jpi-background:tasks:v1",
    jpiBackgroundTasksPayload([{ kind: "task", id: "abc", status: "running" }]),
  );
  ...
  events.emit(
    "jpi-background:response:v1",
    jpiBackgroundStatusResponse({ request_id: "req-1" }, [
  ...
  events.emit(
    "jpi-background:terminal:v1",
    jpiBackgroundTerminalPayload({ kind: "task", id: "abc", status: "completed" }),
  );
...
test("published state merges both providers by namespaced id and each can sustain 'working' alone", async (t) => {
  ...
  harness.events.emit(
    ...
    backgroundResponse(legacyRequest, [{ id: "same", status: "running", isAgent: true }]),
  );
  await harness.extension.flush();
  assert.deepEqual((({ state, agents }) => ({ state, agents }))(await readRecord(path)), {
    state: "working",
    agents: 1,
  });
  ...
  harness.events.emit(
    "jpi-background:response:v1",
    jpiBackgroundStatusResponse(jpiRequest, [{ kind: "task", id: "same", status: "running" }]),
  );
  await harness.extension.flush();
  assert.deepEqual((({ state, agents }) => ({ state, agents }))(await readRecord(path)), {
    state: "working",
...
test("state is unaffected when the jpi-background provider is absent", async (t) => {
  ...
  harness.events.emit(
    "pi-background-tasks:response:v1",
    backgroundResponse(legacyRequest, [{ id: "job", status: "running", isAgent: false }]),
  );
  await harness.extension.flush();
